...
function makeDraggable(w) {
  ...
  const onUp = (e) => {
    ...
    e.stopPropagation(); // не даём ST-обработчикам выше среагировать на этот pointerup
...
// Проглатывает «догоняющие» события после tap-открытия оверлея:
// на мобильных после pointerup браузер стреляет ghost-click (и иногда ghost-pointerdown).
// Используем stopImmediatePropagation — он останавливает ВСЕХ слушателей на этой фазе,
// включая возможные capture-обработчики SillyTavern.
function swallowNextEventsOnce() {
  const EVENTS = ['click', 'pointerdown', 'mousedown'];
  const handlers = {};
  const kill = (e) => {
    // Снимаем все обработчики
    EVENTS.forEach(ev => document.removeEventListener(ev, handlers[ev], true));
    e.preventDefault();
  ...
  // Подстраховка: снять через 600 мс, если события так и не пришли
  setTimeout(() => {
    EVENTS.forEach(ev => document.removeEventListener(ev, handlers[ev], true));
  }, 600);
}
...
function createOverlay() {
  ...
  // Закрытие по тапу на фоне — только pointerdown, не click.
  // click уже «проглочен» swallowNextEventsOnce, а двойной обработчик создаёт гонку.
...
function toggleShop() {
  const now = Date.now();
  // Защита от двойного срабатывания (pointerup + любые другие хендлеры)
  if (now < toggleLockUntil) return;
  toggleLockUntil = now + 700;
